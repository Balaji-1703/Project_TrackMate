...
if os.path.exists(DATA_FILE):
    ...
    df = pd.DataFrame(columns=['name', 'age', 'country', 'subscribed'])

# Create tabs for form and data view
tab1, tab2 = st.tabs(["Submit Form", "View Responses"])
...
# File path for storing attendance
...
# Create tabs for marking attendance and viewing stats
tab1, tab2, tab3 = st.tabs(["Mark Attendance", "View Records", "Statistics"])
...
with tab3:
    ...
    if not df.empty:
        ...
        stats_df = df.groupby('name').agg({
            'status': ['count', lambda x: (x == 'Present').sum()]
        }).reset_index()
        stats_df.columns = ['name', 'total_classes', 'classes_present']
        stats_df['attendance_percentage'] = (stats_df['classes_present'] / stats_df['total_classes'] * 100).round(2)
        ...
        st.bar_chart(stats_df.set_index('name')['attendance_percentage'])
    else:
...
# Create tabs for marking attendance and viewing stats
tab1, tab2, tab3, tab4 = st.tabs(["Mark Today's Attendance", "Mark/Edit Past Attendance", "View Records", "Statistics"])
...
with tab2:
    ...
    # Add new student option
    new_name = st.text_input("Add new student name (Past Attendance)")
    if new_name and new_name not in existing_names:
        existing_names.append(new_name)
...
if 'logged_in_users' not in st.session_state:
    st.session_state.logged_in_users = set()
...
def login():
    ...
    is_admin_attempt = st.sidebar.checkbox("Login as Admin")
    ...
    if is_admin_attempt:
        ...
        viewer_name = st.sidebar.text_input("Enter your name to view records")
        if st.sidebar.button("Enter"):
            if viewer_name:
                st.session_state.current_user = f"viewer:{viewer_name}"
                st.session_state.logged_in_users.add(viewer_name)
                st.sidebar.success(f"Welcome, {viewer_name}")
...
st.sidebar.write(f"Total viewers: {len(st.session_state.logged_in_users)}")
...
if st.sidebar.button("Logout"):
    if st.session_state.current_user:
        ...
        if viewer_name in st.session_state.logged_in_users:
            st.session_state.logged_in_users.remove(viewer_name)
    st.session_state.current_user = None
    st.session_state.is_admin = False
...
if st.session_state.is_admin:
    ...
    tab3, tab4 = st.tabs(["View Records", "Statistics"])
...
def display_active_users():
    ...
    if current_time - st.session_state.last_cleanup > 30:
        cleanup_inactive_users()
            ...
            if st.button("Mark Bulk Attendance"):
                if present_students or absent_students:
                    ...
                    for student in present_students:
                        if len(df[(df['name'] == student) & (df['date'] == today)]) == 0:
                            new_records.append({
                                'name': student,
                                'date': today,
                                'status': 'Present'
                    ...
                    if new_records:
                        ...
                        df.to_csv(DATA_FILE, index=False)
                        st.success(f"Bulk attendance marked for {len(new_records)} students")

                        # Show summary
                        st.write("Summary:")
                        st.write(f"Present: {len(present_students)} students")
                        st.write(f"Absent: {len(absent_students)} students")

                        # Show detailed list
                        if present_students:
                            st.write("Present Students:", ", ".join(present_students))
                        if absent_students:
                            st.write("Absent Students:", ", ".join(absent_students))
                    else:
                        st.warning("Attendance already marked for all selected students today")
                else:
    ...
    if search_name:
        ...
        if not exact_matches.empty:
            ...
            with info_col2:
                ...
                present_days = len(exact_matches[exact_matches['status'] == 'Present'])
                absent_days = len(exact_matches[exact_matches['status'] == 'Absent'])
                ...
                st.metric("Attendance Percentage", f"{attendance_percentage:.1f}%")
            
            # Show attendance trend
            st.divider()
            st.subheader("Attendance History")
            attendance_history = exact_matches.sort_values('date')
            status_numeric = (attendance_history['status'] == 'Present').astype(int)
            st.line_chart(status_numeric)
...
# Create tabs based on user role
if st.session_state.is_admin:
    tab1, tab2, tab3, tab4 = st.tabs(["Mark Attendance", "Mark/Edit Past Attendance", "View Records", "Statistics"])
else:
    tab3, tab4 = st.tabs(["View Records", "Statistics"])
...
if st.session_state.is_admin:
    with tab1:
        ...
        if existing_names:
            # Date selection for bulk attendance
    ...
    with tab1:
        ...
        if existing_names:
            ...
            with col1:
                bulk_date = st.date_input("Select Date for Bulk Attendance", today, key="bulk_date")
            with col2:
                is_special = st.checkbox("Special Class", help="Mark this as a special class session")
                    ...
                    for student in present_students:
                        if student not in existing_records:
                            new_records.append({
                                ...
                                'date': bulk_date,
                                'status': 'Present',
                                'class_type': 'Special' if is_special else 'Regular'
                            })
                        else:
                            skipped_students.append(student)
...
with tab4:
    ...
    if not df.empty:
        ...
        def calculate_stats(data, class_type):
            ...
            stats['attendance_percentage'] = (stats['classes_present'] / stats['total_classes'] * 100).round(2)
            return stats
        ...
        stat_tab1, stat_tab2, stat_tab3 = st.tabs(["Regular Classes", "Special Classes", "Combined"])
